...
'''
use the above optimum threshold to prepare class labels
i.e., all values above the optimum threshold belongs to the 
abnormal class and below belongs to the normal class
'''
custom_y_pred1_opt = np.where(custom_y_pred1 > 0.840677, 1, 0) #change for various operating thresholds
...
custom_y_pred1_platts_opt= np.where(custom_y_pred1_platts > 0.840677, 1, 0)
...
# apply the optimum threshold obtained from the uncalibrated model here
custom_y_pred1_isotonic_opt= np.where(custom_y_pred1_isotonic > 0.840677, 1, 0)
print(custom_y_pred1_isotonic_opt)

#%%
#performance metrics
mat_met = perf_metrix(Y_test1.argmax(axis=-1),
                      custom_y_pred1_isotonic_opt,
                      beta=0.4)
print (mat_met)
...
# apply the optimum threshold obtained from the uncalibrated model here
custom_y_pred1_beta_opt= np.where(custom_y_pred1_beta > 0.840677, 1, 0)
...
# apply the optimum threshold obtained from the uncalibrated model here
custom_y_pred1_spline_opt= np.where(custom_y_pred1_spline > 0.840677, 1, 0)
...
#%%
# apply the optimum threshold obtained from the uncalibrated model here
custom_y_pred1_caliberatedensemble_opt= np.where(custom_y_pred1_caliberatedensemble > 0.840677, 1, 0)
print(custom_y_pred1_caliberatedensemble_opt)

#%%
#performance metrics
mat_met = perf_metrix(Y_test1.argmax(axis=-1),
                      custom_y_pred1_caliberatedensemble_opt,
                      beta=0.4)
print (mat_met)
