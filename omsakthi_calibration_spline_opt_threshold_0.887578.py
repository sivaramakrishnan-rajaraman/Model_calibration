...
We obtained the same optimal threshold from the ensemble as well
0.9574766891736715
Best Threshold=0.887578, G-Mean=0.894
Best Threshold=0.887578, Youden J statistic=0.789
So we are going to use this threshold now for the third table in the 
Results Section
'''

custom_y_pred1_spline_opt = np.where(custom_y_pred1 > 0.887578, 1, 0) #change for various operating thresholds
...
custom_y_pred1_platts_spline_opt= np.where(custom_y_pred1_platts > 0.887578, 1, 0)
print(custom_y_pred1_platts_spline_opt)

#%%
#compute performance metrics after thresholding the Platt scaling

mat_met = perf_metrix(Y_test1.argmax(axis=-1),
                      custom_y_pred1_platts_spline_opt,
                      beta=0.4)
print (mat_met)
...
# apply the optimum threshold obtained from the uncalibrated model here
custom_y_pred1_isotonic_spline_opt= np.where(custom_y_pred1_isotonic > 0.887578, 1, 0)
print(custom_y_pred1_isotonic_spline_opt)

#%%
#performance metrics
mat_met = perf_metrix(Y_test1.argmax(axis=-1),
                      custom_y_pred1_isotonic_spline_opt,
                      beta=0.4)
print (mat_met)
...
# apply the optimum threshold obtained from the uncalibrated model here
custom_y_pred1_beta_spline_opt= np.where(custom_y_pred1_beta > 0.887578, 1, 0)
print(custom_y_pred1_beta_spline_opt)

#%%
#performance metrics
mat_met = perf_metrix(Y_test1.argmax(axis=-1),
                      custom_y_pred1_beta_spline_opt,
                      beta=0.4)
print (mat_met)
...
# apply the optimum threshold obtained from the uncalibrated model here
custom_y_pred1_spline_spline_opt= np.where(custom_y_pred1_spline > 0.887578, 1, 0)
print(custom_y_pred1_spline_spline_opt)

#%%
#performance metrics
mat_met = perf_metrix(Y_test1.argmax(axis=-1),
                      custom_y_pred1_spline_spline_opt,
                      beta=0.4)
print (mat_met)
...
#%%
# apply the optimum threshold obtained from the uncalibrated model here
custom_y_pred1_caliberatedensemble_spline_opt= np.where(custom_y_pred1_caliberatedensemble > 0.887578, 1, 0)
print(custom_y_pred1_caliberatedensemble_spline_opt)

#%%
#performance metrics
mat_met = perf_metrix(Y_test1.argmax(axis=-1),
                      custom_y_pred1_caliberatedensemble_spline_opt,
                      beta=0.4)
print (mat_met)
